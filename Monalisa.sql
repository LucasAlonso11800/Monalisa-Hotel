...
CREATE TABLE `room_amenities` (
    ...
    PRIMARY KEY (
        `room_ameniti_room_category_id`,
        `room_ameniti_ameniti_id`
    )
);
...
CREATE TABLE `room_prices` (
    ...
    PRIMARY KEY (
        `room_price_room_category_id`,
        `room_price_price_category_id`
    )
);
...
CREATE TABLE `reserves` (
    ...
    `reserve_owner_name` varchar(40) 
    `reserve_owner_last_name` varchar(40),
    `reserve_owner_phone` int,
    `reserve_owner_email` varchar(255) 
    `reserve_owner_country` varchar(40),
    `reserve_owner_notes` text,
    `reserve_owner_zip` varchar(20),
    `reserve_passengers` int
);
...
ALTER TABLE
    `rooms`
ADD
    FOREIGN KEY (`room_room_category_id`) REFERENCES `room_categories` (`room_category_id`);

ALTER TABLE
    `room_amenities`
ADD
    FOREIGN KEY (`room_ameniti_room_id`) REFERENCES `room_categories` (`room_category_id`);

ALTER TABLE
    `room_amenities`
ADD
    FOREIGN KEY (`room_ameniti_ameniti_id`) REFERENCES `amenities` (`ameniti_id`);

ALTER TABLE
    `room_prices`
ADD
    FOREIGN KEY (`room_price_room_category_id`) REFERENCES `room_categories` (`room_category_id`);

ALTER TABLE
    `room_prices`
ADD
    FOREIGN KEY (`room_price_price_category_id`) REFERENCES `price_categories` (`price_category_id`);

ALTER TABLE
    `room_reserves`
ADD
    FOREIGN KEY (`room_reserve_room_id`) REFERENCES `rooms` (`room_id`);

ALTER TABLE
    `room_reserves`
ADD
    FOREIGN KEY (`room_reserve_reserve_id`) REFERENCES `reserves` (`reserve_id`);